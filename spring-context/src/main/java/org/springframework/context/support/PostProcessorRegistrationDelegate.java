...
final class PostProcessorRegistrationDelegate {
	 ...
	 *
	 * 1. BeanPostProcessors是spring提供在外的拓展点；允许在容器初始化bean前后执行用户自己的逻辑。
	 * 2. 这里是注册所有实现了BeanPostProcessors的类；仅是注册
	 *
	 * 注册的处理器执行时机
	 * > 用得是职责链调用的方式；将chain里的所有类的方法都走一遍
	 * 在所有 bean 实例化时，
	 * 1. 执行初始化方法前会调用所有 BeanPostProcessor 的 postProcessBeforeInitialization 方法
	 * 2. 在执行初始化方法后会调用所有 BeanPostProcessor 的 postProcessAfterInitialization 方法
	 *
